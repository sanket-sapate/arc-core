...
import { useVendors, useCreateVendor, useUpdateVendor, useDeleteVendor, createVendorDPA } from "~/features/tprm/api/vendors";
                ...
                onSuccess: async (newVendor) => {
                    try {
                        if (data.requires_dpa && newVendor?.id) {
                            await createVendorDPA(newVendor.id);
                        }
                        ...
                        if (data.requires_assessment) {
                            toast.info("Head to Assessments to link a compliance framework.", { duration: 5000 });
                        }
                    } catch (err) {
                        toast.error("Vendor created, but failed to initialize the DPA.");
                    }
                },
                onError: (err) => {
                ...
                <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
                    <SheetContent className="overflow-y-auto sm:max-w-[800px] w-full p-4 sm:p-6">
                        <SheetHeader className="mb-6">
                            <SheetTitle>{editingVendor ? "Edit Vendor" : "Onboard New Vendor"}</SheetTitle>
                            <SheetDescription>
                                {editingVendor
                                    ? "Update external processor details and compliance posture."
                                    : "Complete the onboarding wizard to register a new third-party vendor."}
                            </SheetDescription>
                        </SheetHeader>
                        <VendorEditor
