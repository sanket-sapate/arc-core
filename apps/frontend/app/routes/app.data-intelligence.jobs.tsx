...
import {
    ...
    Filter, RefreshCw, Trash2, ChevronLeft, BarChart3,
} from "lucide-react";
...
export default function JobsPage() {
    ...
    const { data: jobs, isLoading, isError } = useJobs(page, 50);
    ...
    const completedJobs = jobs?.filter((j) => j.status?.toUpperCase() === "COMPLETED").length ?? 0;
    const totalFindings = jobs?.reduce((acc, j) => acc + (j.findings_count ?? 0), 0) ?? 0;
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Card>
                    ...
                    <CardContent>
                        <div className="text-3xl font-bold">{isLoading ? "—" : totalJobs}</div>
                    </CardContent>
                </Card>
                    ...
                    <Table>
                        ...
                        <TableBody>
                            {filteredJobs.length === 0 ? (
                                <TableRow><TableCell colSpan={7} className="h-32 text-center text-muted-foreground">{searchQuery || statusFilter !== "all" ? "No jobs match the current filters." : "No scan jobs yet. Trigger a scan from the Data Sources page."}</TableCell></TableRow>
                            ) : (
