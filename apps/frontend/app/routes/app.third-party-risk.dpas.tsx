...
import { DPAEditor, type DPASubmitPayload } from "~/features/tprm/components/DPAEditor";
import { useDPAs, useCreateDPA, useUpdateDPA, useDeleteDPA, addDPADataScope } from "~/features/tprm/api/dpas";
...
export default function DPAsPage() {
    ...
    const handleSubmit = (data: DPASubmitPayload) => {
        const { data_object_ids, ...dpaData } = data;
        if (editingDPA) {
            updateDPA.mutate(
                { id: editingDPA.id!, dpa: dpaData },
                {
                    onSuccess: () => {
            ...
            createDPA.mutate(dpaData, {
                onSuccess: async (newDpa) => {
                    if (data_object_ids?.length && newDpa?.id) {
                        try {
                            await Promise.all(
                                data_object_ids.map((dictId) => addDPADataScope(newDpa.id!, dictId))
                            );
                        } catch {
                            toast.error("DPA created, but some data objects failed to attach.");
                        }
                    }
                    setIsSheetOpen(false);
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                ...
                <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
                    <SheetContent className="overflow-y-auto sm:max-w-[800px] w-full p-4 sm:p-6">
                        <SheetHeader className="mb-6">
                            <SheetTitle>{editingDPA ? "Edit Agreement" : "New Data Processing Agreement"}</SheetTitle>
                            <SheetDescription>
                                {editingDPA
                                    ? "Update DPA details, scope, and contract terms."
                                    : "Complete the wizard to create a new DPA with scope, sub-processors, and security terms."}
                            </SheetDescription>
                        </SheetHeader>
                        <DPAEditor
